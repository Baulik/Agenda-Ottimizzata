# VERSIONE: 2.3.2 (Stabile - Ripristino Totale: Date, Isole di Tempo e Bug Fix)
...
def get_fascia_info(ora_str):
    try:
        ...
        t = h + m/60
        # Fasce flessibili per non perdere appuntamenti
...
st.set_page_config(page_title="Analisi Appuntamenti 2.3.2", layout="wide")
st.markdown("""
<style>
    thead tr th:first-child {display:none} tbody th {display:none}
    .year-card { border: 1px solid #dcdde1; border-radius: 12px; padding: 10px; background-color: #ffffff; margin-bottom: 20px; box-shadow: 0 4px 6px rgba(0,0,0,0.05); }
    ...
    .day-label { width: 22%; font-weight: bold; font-size: 10px; color: #7f8c8d; text-transform: uppercase; text-align: left !important; }
...
if raw_data:
    ...
    if not df_week.empty:
        ...
        fasce_tot = ["09:00-12:00", "12:30-15:30", "16:00-19:00", "19:30-22:00"]
        counts = df_week['Fascia'].value_counts()
        piu_libera = min(fasce_tot, key=lambda x: counts.get(x, 0))
        piu_carica = max(fasce_tot, key=lambda x: counts.get(x, 0))
        
        st.markdown(f"""
        <div class='insight-box'>
            <b>💡 Analisi Predittiva Settimana {sel_week}:</b><br>
            • <b>Frequenza Media:</b> In questa settimana hai mediamente <b>{media_app}</b> appuntamenti/anno.<br>
        ...
        for idx, anno in enumerate(anni):
            with cols[idx]:
                ...
                st.markdown(f"<div class='year-card'><h3 style='text-align:center;'>{anno}</h3><table class='grid-table'><tr style='font-size:8px; color:#95a5a6;'><th>Giorno</th><th>09-12</th><th>12:3-15</th><th>16-19</th><th>19:3-22</th></tr>{rows_html}</table></div>", unsafe_allow_html=True)
    ...
    pivot_f = pivot_f.reindex(index=["Lun","Mar","Mer","Gio","Ven","Sat"], columns=ordine_f).fillna(0).astype(int)
